...
def test_caddyfile_never_exposes_the_scan_orchestrator_api():
    # 2안: 검사 요청은 MCP Gateway의 loopback 게시(SCAN_API_PORT)로만, 관리자는 SSH 터널로 쓴다.
    handles = _handles()
    assert "respond 404" in handles["/scans*"]
    assert not any("mcp-gateway" in body or "aig" in body for body in handles.values())


NEVER_EXPOSED_SERVICES = {"aig-web", "aig-agent", "test-mcp", "test-api", "trivy"}


def test_aig_and_security_test_zone_never_join_the_edge_network():
    compose = yaml.safe_load((ROOT / "compose.yaml").read_text(encoding="utf-8"))
    for name in NEVER_EXPOSED_SERVICES:
        service = compose["services"][name]
        assert "edge" not in service.get("networks", []), f"{name} must not join the edge network"
        assert not service.get("ports"), f"{name} must not publish a host port"
    field = yaml.safe_load((ROOT / "compose.field.yaml").read_text(encoding="utf-8"))
    assert not NEVER_EXPOSED_SERVICES & set(field["services"]), "the field overlay must not touch the scan zone"
