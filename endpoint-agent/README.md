...
1. Gateway Console의 **엔드포인트 → 장치 등록**에서 고유한 `endpoint_id`와 `inventory` 권한의 장치 키를 발급합니다. 망 탐색이 필요하고 정책 범위가 승인됐다면 `netscan`도 추가합니다. 키는 한 번만 표시됩니다.
...
- Console의 **엔드포인트** 화면에서 등록 장치, 마지막 보고 시각, `registered` / `shadow` / `retired-residue` 분류를 확인합니다. `--once`가 0으로 끝났다는 사실만으로 수집 범위가 충분하다는 뜻은 아닙니다.
- 장치 키를 잃거나 PC를 폐기하면 Console에서 그 장치 자격을 폐기합니다. 새 키가 필요하면 재발급 후 재설치합니다.
...
- 코드 자체 점검: `python endpoint-agent/agent.py --self-check`. Compose 한 PC 실습은 `cd full_stack_lab && ./console.sh endpoint`를 사용합니다.
