...
기존 공용 `llm-stub`은 A.I.G의 큐·CLI·native JSON/SARIF 결과 경로를 검증하는 test double입니다. 별도 A.I.G 엔진이 아니며 결과에 `evidence_mode: test-double`이 남고 Gateway 차단 근거에는 들어가지 않습니다. `./console.sh local-llm`은 실제 Ollama 모델을 `model` 내부망에 올리고 A.I.G Web에 자동 등록하지만, 소형 모델의 검사 결과는 `evidence_mode: advisory`로 기록합니다. 기존 live 결과는 test-double/advisory 검사 때문에 삭제되지 않습니다. 실제 차단 근거에 넣으려면 검증된 모델의 독립 발견 결과를 확인하고 `MCP_SCAN_EVIDENCE_MODE=live`로 실행해야 합니다.
