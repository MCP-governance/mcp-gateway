...
function renderTop() {
  ...
  $("b", wire).textContent = healthLabel(health);
...
function renderFeed() {
  ...
  $("#strip").innerHTML = rows.map((row) => `<i data-v="${esc(row.decision)}" style="height:${row.upstream_executed ? 30 : 16}px"></i>`).join("");
...
function wire() {
  ...
  intakeForm.addEventListener("submit", async (event) => {
    ...
    if (created) { form.reset(); $("#purpose-n").textContent = "0"; exitTermsVerdict(form); }
  });
