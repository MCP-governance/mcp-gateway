...
망의 **실제 MCP 서버 10종** 중 하나로 전달한다. 웹 Console은 관제·승인·종료 판정만 하고
**MCP를 호출하지 않는다.**
...
```text
        ...
        :8000 Console·IdP     :8080 Gateway API     :16686 Jaeger
             │                     │
┌────────────┼─────────── office (internal) ──────────────────────────────┐
│  ws-ysg  ws-jwj  ws-pse  ws-nkk   ← 직원 워크스테이션(office-agent + endpoint-agent)
│     │  ① OAuth 토큰(IdP)   ② LLM 호출(LiteLLM 가상 키)   ③ MCP 호출(Gateway /mcp)
│     ▼                         ▼                           ▼
│  agent-service(IdP·Console)  llm-gateway(LiteLLM) ──model──► ollama
│                               gateway ──policy──► opa
└───────────────────────────────┼──────────────────────────────────────────┘
                                │ tools (internal) — 직원 컨테이너는 이 망에 없다
...
| 망 | 붙는 서비스 | 의미 |
| --- | --- | --- |
| `edge` | agent-service, gateway, jaeger | 호스트 loopback 게시 전용 |
| `office` | 워크스테이션, agent-service, gateway, llm-gateway | 직원 PC가 닿는 유일한 망 |
| `tools` | gateway, mcp-* | **Gateway만** MCP 서버에 닿는다 |
| `corp` | mcp-*(필요한 것만), corp-*, intranet, corp-seed | MCP 서버의 하위 시스템 |
| `ops` | gateway, corp-git | 종료 판정 증거 수집(Gitea 관리 API) |
| `internet` | external-web, mcp-fetch, mcp-playwright | 외부 인터넷 모사(유출 목적지) |
| `model` | ollama, llm-gateway, intake-worker | 추론 |
| `data` / `policy` / `telemetry` | 기존과 동일 | |
...
5. `classify.py`가 인자에서 **자원**(경로·URL·SQL 테이블·저장소·수신자·명령·키)을 뽑아 등급
   ...
   `SELECT`는 r, `UPDATE`는 w, `DROP`은 x.
6. OPA가 333 매트릭스와 MCP-* 통제를 판정한다: Allow / Alert / Restrict / Approval / Block.
7. 허용이면 같은 연결에서 계약(도구 설명·스키마·버전 해시)을 다시 확인하고 upstream에 전달한다.
8. 결과는 크기·주입 표지 검사를 거쳐 돌려준다. 판정은 해시 체인 감사 원장에 남는다.
...
stdio 전용 서버는 `mcp-proxy`(0.12.0, `--stateless`)가 Streamable HTTP로 바꿔 준다.
버전은 이미지 빌드 시점에 고정하고, 계약 해시는 `registry/contracts.lock.json`에 커밋한다
...
```json
{
  ...
  "resource": {"kind": "path", "id": "/shared/confidential/salary-2026.csv", "data_class": "important"},
  "resources": [ ... ],
  "destinations": [{"kind": "email", "value": "x@gmail.com", "external": true}],
  "request": {"untrusted_markers": [], "dlp": ["kr-rrn"]},
  "contract": { ... }, "context": { ... }, "approval": { ... }
}
```
...
| Console 사용자 | 로그인 세션 JWT | 기존 `/auth/mock-login` |
| 워크스테이션 장치 | 엔드포인트 장치 키 | 기존 엔드포인트 평면 |
...
보호 자원 메타데이터를 낸다. 종료 판정의 증거 수집이 이 표준 엔드포인트를 그대로 쓴다.
...
- gitea-mcp의 PAT는 게이트웨이 차단 후에도 Gitea에 유효하다(E3). 조직이 Gitea 관리자로서 폐기·확인할
  수 있으면 증거가 자동 수집되어 T1까지 오를 수 있고, 아니면 T3에 머문다.

## 8. 디렉터리

| 경로 | 내용 |
| --- | --- |
| `full_stack_lab/gateway/app/` | Gateway(FastAPI)·IdP·Console API |
| `full_stack_lab/gateway/app/classify.py` | 자원 추출·분류·DLP |
| `full_stack_lab/gateway/app/upstream.py` | 등록 서버로의 Streamable HTTP 연결과 계약 재확인 |
| `full_stack_lab/registry/` | `catalog.toml`(서버·분류·이용 관계), `contracts.lock.json` |
| `full_stack_lab/mcp/` | MCP 런타임 이미지와 서버별 실행 설정 |
| `full_stack_lab/corp/` | 회사 시드 데이터(파일·저장소·DB·인트라넷·메일·Gitea) |
| `full_stack_lab/workstation/` | 직원 워크스테이션 이미지, `office_agent.py`, 시나리오 |
| `full_stack_lab/llm/` | LiteLLM 설정, 직원 키 발급 |
| `full_stack_lab/experiments/` | 논문 E1·E2·E3 재현 스크립트 |
