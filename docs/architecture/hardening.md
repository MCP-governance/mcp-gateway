...
> **v2 반영 현황 (2026-09-25 통합)** — 이 제안은 v1 기준 커밋(45b9f6e)의 정적 검토다. v2(직원 PC 에이전트 +
> 실제 MCP 서버 10종) 통합 때 반영한 것: 승인 최종 상태의 미확인 실행 구분(D-27), 권한 데이터를 포함한 배포 정책
> 묶음 digest(D-25). 남은 것(실행 journal, DB 역할 분리, 순수 계약 모듈, digest의 감사·재생 연결)은
> [docs/ai/ROADMAP.md](../ai/ROADMAP.md) 5절. 본문의 파일·행 번호는 v1 기준이다.
