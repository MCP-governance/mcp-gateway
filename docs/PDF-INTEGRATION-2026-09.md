...
> **v2 이식 (2026-09-25).** 이 문서의 본문은 v1(모의 MCP 서버, `send_external`·`read_document` 도구) 위에서
> 작성된 기록이다. v2 병합에서 같은 통제를 실제 서버 구조로 옮겼고, 달라진 점은 다음과 같다.
>
> | 항목 | v1 (이 문서 본문) | v2 |
> | --- | --- | --- |
> | 외부 목적지 판단 | `send_external`의 `destination`과 `data.egress.internal_domains` | 분류기가 모든 도구 인자에서 뽑는 `destinations[].external`(카탈로그의 사내 도메인·인트라넷 기준) |
> | `MCP-DATA-EGRESS-001` | `send_external`에서만 | 외부 목적지가 있는 **모든** 호출(외부 메일·외부 URL 등) |
> | `P-CHAIN-001` 연결 단위 | 서명된 Agent 세션 id | **검증된 주체**(토큰의 사용자). 클라이언트가 작업을 새로 열어 쪼갤 수 없다 |
> | Presidio 검사 범위 | 문서 본문 | 쓰기·실행이거나 외부 목적지가 있는 호출의 인자 전체, 그리고 **모든 실행 결과** |
> | 검사 엔터티 | Presidio 기본 인식기 전체 + 한국 식별자 | 규제 식별자·자격만(`EMAIL_ADDRESS`, `KR_RRN`, `KR_PHONE`, `CREDENTIAL`, `CREDIT_CARD`, `IBAN_CODE`, `US_SSN`). 실제 도구 출력(git 로그·SQL·웹)에서 날짜·이름·URL을 가리지 않기 위해 |
> | 이메일 예외 | 없음 | 호출의 수신자 주소, 사내 도메인(`bob.local`) 주소는 개인정보 유형으로 세지 않고 가리지 않는다 |
> | 네트워크 | 전용 `privacy` 망 | OPA와 같은 `policy` 망(주소 풀 소진 회피, Gateway만 접근) |
> | 감사 체인 | v5 = v4 + 정책 입력·위험 점수·개인정보 유형·연쇄 표지 | v2의 v5(서버·자원·목적지·클라이언트·요약)에 이어 **v6**로 기록 |
> | 정책 재생 사례 | v1 도구 입력 | v2 입력 형태(실제 서버·도구, `destinations`)의 합성 10사례 |
>
> 현재 설계와 검증은 [ai/ARCHITECTURE.md](ai/ARCHITECTURE.md), [ai/POLICY.md](ai/POLICY.md), [ai/TESTING.md](ai/TESTING.md),
> 결정 이유는 [ai/DECISIONS.md](ai/DECISIONS.md) D-20~D-23.
...
flowchart LR
    ...
    T --> A[Audit API / Runtime Evidence]
    A --> R
    W[격리 검증 워커: A.I.G · Trivy · Syft · Semgrep] --> R
    C[후보 OPA + 동결 사례 재생] -. 정책 입력만 .-> O
```

draw.io의 Gateway, OPA, 스캐너, AI-Infra-Guard, OTel, 증적 분석·저장, 대시보드 구성을 현재 서비스에 대응시켰다. 정책 증적과 runtime span은 Audit API가 PostgreSQL에 정규화하고 Console이 조회한다. 모델은 도구를 제안하고 실행 권한은 Gateway와 OPA가 결정한다.
...
| 5~8쪽 정책·연쇄·위험 | OPA의 `Allow/Alert/Approval/Restrict/Block`과 배포형 권한 규칙 | Tool 수신 도메인에 대한 `MCP-DATA-EGRESS-001`, 동일 서명 세션의 중요 열람→전송에 대한 `P-CHAIN-001`, 0~100 위험 점수 추가. 점수는 조사용 증적이며 허가 근거는 구체적인 Rego 조건 |
...
| 10~12쪽 강제 경로·감사 | Gateway 전용 MCP 네트워크, PostgreSQL append-only 감사, 독립 효과 로그, OTel Collector·Audit API | 감사 체인 v5에 정책 입력 스냅샷·위험 점수·PII 유형·연쇄 표지 포함. runtime span은 허용된 운영 메타데이터만 별도 테이블에 저장. 상태 API에 Presidio 상태 포함 |
...
| [OpenTelemetry](https://opentelemetry.io/docs/), [PostgreSQL](https://www.postgresql.org/docs/) | Collector가 trace 메타데이터를 Audit API로 전달하고 PostgreSQL이 정책·runtime 증적을 보존 |
